...
const ChatContainer = () => {
    ...
    const { messages, selectedUsers, sendMessage, getMessages, setSelectedUsers } = useChat();
        ...
        <div className='h-full overflow-scroll relative backdrop-blur-lg'>
            ...
            <div className='flex items-center gap-3 py-3 mx-4 border-b border-stone-500'>
                ...
                <p className='flex-1 text-lg text-white flex items-center gap-2'>
                    ...
                    {onlineUser.includes(selectedUsers._id) && <span className='w-2 h-2 rounded-full bg-green-500'></span>}
                </p>
            ...
            <div className='absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3'>
                <div className='flex-1 flex items-center bg-gray-100/12 px-3 rounded-full'>
                    <input type="text" onChange={(e) => setInput(e.target.value)} value={input}
                        onKeyDown={(e) => e.key === "Enter" ? handleSendMessage(e) : null} placeholder='Send a messsge' className='flex-1 text-sm p-3 border-none rounded-lg outline-none text-white placeholder-gray-400' />
                    <input onChange={handleSendImage} type="file" id="image" accept='image/png, image/jped' hidden />
                    <label htmlFor="image">
